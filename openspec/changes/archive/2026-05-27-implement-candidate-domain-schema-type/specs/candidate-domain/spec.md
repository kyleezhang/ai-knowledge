...
系统 SHALL 在本变更中只实现 Candidate domain contract，不得接入 Candidate storage、CLI、collector、workflow、Source conversion、indexing 或 answer retrieval。
...
- **AND** 不新增 `candidate` CLI 命令
- **AND** 不新增 Candidate repo 或写入 `knowledge/candidates/` 的 workflow
- **AND** 不把 Candidate 写入 main index
