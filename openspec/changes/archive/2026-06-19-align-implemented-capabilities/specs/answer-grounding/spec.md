...
- **WHEN** the user asks a question without explicitly enabling fallback and no approved index entry matches it
- **THEN** the system states that there is no related confirmed knowledge
- **AND** does not call raw Source, `draft_understanding`, discussion summary, Candidate, vector chunk text, or other unconfirmed material as if it were approved knowledge
...
- **THEN** the answer may include eligible unconfirmed materials as secondary evidence
- **AND** approved Notes remain primary evidence whenever available
- **AND** every unconfirmed material is labeled separately from approved Note evidence

### Requirement: Unapproved Material Is Secondary Evidence
The system SHALL NOT use raw sources, draft understanding, discussion summaries, Candidates, or vector chunk text as secondary evidence in default answer workflow. When fallback is explicitly enabled, the system MAY use structured unconfirmed materials as secondary evidence, but MUST label them as unconfirmed and MUST NOT use raw artifacts or Candidates.

#### Scenario: Approved notes are insufficient
- **WHEN** approved notes do not sufficiently answer the question and fallback is not explicitly enabled
- **THEN** the P0 Stable system reports limitations
- **AND** does not consult raw sources, draft understanding, discussion-stage material, Candidates, or vector chunk text
...
- **AND** keeps them separate from approved Note evidence in workflow data and output
...
The system SHALL avoid fabricating conclusions when available knowledge is insufficient. Claims based on fallback evidence MUST be phrased as unconfirmed and MUST include limitations. Default answers MUST NOT use model background knowledge as project knowledge.
...
- **WHEN** the Answer Agent is called in default P0 Stable mode
...
- **AND** must include limitations for those unconfirmed claims

### Requirement: Retrieval Uses Main Index As Entry Point
The system SHALL use the main index as the entry point for approved-note retrieval in default P0 Stable retrieval and P3 Experimental hybrid retrieval. P0 Stable retrieval searches keyword and metadata index entries. When hybrid retrieval is explicitly enabled, retrieval MAY also use vector references reachable from approved Index Entries.

#### Scenario: User asks a question
- **WHEN** the answer workflow begins retrieval in default mode
...
- **AND** distinguishes approved notes from unavailable or unapproved material
